...
import llm_tree.llm
import llm_tree.tree
import random
import imodelsx.data
import llm_tree.ensemble
...
def test_stump_always_improves_acc():
    for classification_or_regression in ['classification', 'regression']: # classification, regression
        stump_class = llm_tree.stump.StumpClassifier if classification_or_regression == 'classification' else llm_tree.stump.StumpRegressor
        for seed in range(2):
...
def test_tree_monotonic_in_depth(refinement_strategy='None', max_features=1, embs_manager=None):
    for classification_or_regression in ['regression', 'classification']:
        ...
        if classification_or_regression == 'classification':
            tree_class = llm_tree.tree.TreeClassifier
        else:
            tree_class = llm_tree.tree.TreeRegressor
...
def test_tree_ensemble(
    ...
    tree = llm_tree.tree.TreeClassifier(
        max_depth=max_depth,
        split_strategy='cart',
        max_features=max_features,
        verbose=False,
    ...
    m = llm_tree.ensemble.BaggingEstimatorText(tree, n_estimators=n_estimators)
